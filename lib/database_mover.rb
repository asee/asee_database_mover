...
module ASEE
  class DatabaseMover
    ...
    def initialize(prj, cnf, src, tgt, dry_run = false, debug = 0)
      ...
      @src_deps = cnf[prj][src].has_key?('deps') ? cnf[prj][src]['deps'] : ['applicants', "#{@prj}_awards", 'universities'].map {|x| "#{x}_#{src}"}
      @tgt_deps = cnf[prj][tgt].has_key?('deps') ? cnf[prj][tgt]['deps'] : ['applicants', "#{@prj}_awards", 'universities'].map {|x| "#{x}_#{tgt}"}
